use postcard::{Deserializer, de_flavors::io::io::IOReader};
...
// TODO: Both of these are horribly inneficient
// fix the buffer on decode and make it async, also make encode() write directly
// without allocating an extra Vec
impl<T> AkarekoWrite for T
where
    T: Serialize,
{
...
//     fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) -> Result<Self,
// DecodeError>     where
//         Self: Sized;
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(_reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(())
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         let b =
// uuid::Bytes::decode(reader).await?;         Ok(Uuid::from_bytes(b))
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok((T::decode(reader).await?,
// U::decode(reader).await?))     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_u8().await?)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_u16().await?)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_u32().await?)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_u64().await?)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         let value = reader.read_u8().await?;
//         if value == 0 {
//             Ok(None)
//         } else {
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         let len = reader.read_u16().await?;
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_u8().await? != 0)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_f32().await?)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         let mut buf = [0u8; N];
//         reader.read_exact(&mut buf).await?;
//         Ok(buf)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_i32().await?)
//     }
// }
...
//     async fn decode<R: AsyncRead + Unpin + Send>(reader: &mut R) ->
// Result<Self, DecodeError> {         Ok(reader.read_i64().await?)
//     }
// }
